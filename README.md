...
## 🌱 Структура проекта:
- `build/` — тут лежать готові програми для платформ (EXE+DMG+APK)
...
## 🖼 Як це запустити легко і зручно на WINDOWS ?:
...
## 🍎 Як це запустити легко і зручно на MacOS ?:
1. скачуємо файл `build/KeySwitch_dmg.dmg`
2. запускаємо цей файл подвійним нажаттям мишки на ньому
3. перетягуємо додаток в Applications

## 📲 Як це запустити легко і зручно на ANDROID ?:
1. скачуємо на телефон файл `build/keyswitch-2.0-arm64-v8a_armeabi-v7a-debug.apk`
2. запускаємо цей файл нажаттям на ньому

## 😎 Як це запустити із ісходних файлів на UBUNTU ?:
